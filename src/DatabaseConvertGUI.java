...
public class DatabaseConvertGUI {
   ...
   private IFileParserInput ecfp;	//INTERFACES FOR CHANGE
   private IFileParserOutput eccd;  //INTERFACES FOR CHANGE
   
   private static PrintWriter pw;
   private DatabaseTable[] tables; //master copy of EdgeTable objects
   private DatabaseField[] fields; //master copy of EdgeField objects
   private DatabaseTable currentDTTable, currentDRTable1, currentDRTable2; //pointers to currently selected table(s) on Define Tables (DT) and Define Relations (DR) screens
   private DatabaseField currentDTField, currentDRField1, currentDRField2; //pointers to currently selected field(s) on Define Tables (DT) and Define Relations (DR) screens
   ...
   public void createDTScreen() {//create Define Tables screen
      ...
      strDataType = DatabaseField.getStrDataType(); //get the list of currently supported data types
            ...
            public void actionPerformed(ActionEvent ae) {
               ...
               try {
                  if (result.length() > 5) {
                     ...
                     jtfDTVarchar.setText(Integer.toString(DatabaseField.VARCHAR_DEFAULT_LENGTH));
                     return;
                  }
   ...
   private void getOutputClasses() {
      ...
      Class[] paramTypes = {DatabaseTable[].class, DatabaseField[].class};
      ...
      try {
         for (int i = 0; i < resultFiles.length; i++) {
         	//ignore all files that are not .class files
            ...
            if (resultClass.getSuperclass() == null){
            	continue; //For Interfaces
            }
            
            if (resultClass.getSuperclass().getName().equals("FileParserOutput")) { //only interested in classes that extend EdgeConvertCreateDDL
   ...
   private String getSQLStatements() {
      ...
      if (response == null) {
         return DatabaseConvertGUI.CANCELLED;
      }
   ...
   class CreateDDLButtonListener implements ActionListener {
      public void actionPerformed(ActionEvent ae) {
         ...
         if (sqlString.equals(DatabaseConvertGUI.CANCELLED)) {
            return;
         }
         writeSQL(sqlString);
      }
      ...
      public void actionPerformed(ActionEvent ae) {
         ...
         if ((ae.getSource() == jmiDTOpenSave) || (ae.getSource() == jmiDROpenSave)) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                * WE NEED TO CHANGE THIS - TODO WORK
                */
               ecfp = new EdgeFileParser(saveFile);
               /*
                * JUST MAKE THE ABOVE FILE CHOOSE THE RIGHT ONE!!
                */               
